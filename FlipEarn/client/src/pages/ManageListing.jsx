        ...
        <form onSubmit={handleSubmit} className='space-y-8'>
          ...
          <Section title="Account Metrics">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <InputField label='Followers Count *' type='number' min={0} 
              value={formData.followers_count} placeholder='10000' onChange={(v) => handleInputChange('followers_count', v)} required={true}/>

              <InputField label='Engagement Rate (%)' type='number' min={0} max={100}
              value={formData.engagement_rate} placeholder='4' onChange={(v) => handleInputChange('engagement_rate', v)}/>

              <InputField label='Monthly Views/Impressions' type='number' min={0} 
              value={formData.monthly_views} placeholder='100000' onChange={(v) => handleInputChange('monthly_views', v)}/>
          ...
          <Section title='Pricing & Description'>
            <InputField label='Asking Price (USD) *' type='number'
            min={0} value={formData.price} placeholder='2500.00'
                ...
                {formData.images.map((img, index)=>(
                  <div key={index} className="relative">
                    <img src={typeof img === 'string' ? img : URL.createObjectURL(img)}
                     alt={`image ${index + 1}`} className='w-full h-24
                     object-cover rounded-lg'/>
                     <button type='button' onClick={()=> removeImage(index)}
                      className='absolute -top-2 -right-2 size-6 bg-red-600
                      text-white rounded-full hover:bg-red-700'>
                      x
                     </button>
                  </div>
                ))}
          ...
          <div className='flex justify-end gap-3 text-sm'>
            <button onClick={()=>navigate(-1)} type='button' className="px-6
            py-2 border border-gray-300 rounded-lg hover:bg-gray-50
            transition-colors">
              Cancel
            </button>
            <button type='submit' className="px-6
            py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700
            transition-colors">
...
const InputField = ({ label, value, onChange, placeholder, type = 'text', required = false, min = null, max = null})=>(
  <div>
    ...
    <input type={type} min={min} max={max} placeholder={placeholder} value={value}
    onChange={(e)=>onChange(e.target.value)} className="w-full px-3 py-1.5
    text-gray-600 border rounded-md focus:outline-none focus:ring-2
