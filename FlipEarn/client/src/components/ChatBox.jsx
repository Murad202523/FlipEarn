        ...
        (
          <form onSubmit={handleSendMessage} className='p-4 bg-white border-t border-gray-200 rounded-b-lg'>
            <div className='flex items-end space-x-2'>
              ...
              <button disabled={!newMessage.trim() || isSending} type='submit'
              className="bg-indigo-600 hover:bg-indigo-700
              text-white p-2.5 rounded-lg disabled:opacity-50
              transition-colors">
                {isSending ?
                <Loader2Icon className='w-5 h-5 animate-spin'/> : <Send className="w-5 h-5"/>}
              </button>
            </div>
          </form>
        )
