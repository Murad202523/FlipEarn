      ...
      <div className="bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-[320px] flex flex-col">
        ...
        gap-4 p-4 overflow-y-scroll'>
          {credential.map((cred, index)=>(
            <div key={cred.type} className="grid grid-cols-[2fr_3fr_1fr]
            items-center gap-2">
              <label className="text-sm font-medium text-gray-800">{cred.name}
              </label>
              <input type="text" value={cred.value} onChange={(e)=>setCredential((prev)=>prev.map((c, i)=>(i===index ? {...c, value: e.target.value} : c)))}
          ...
          <div className="flex items-center gap-2">
            ...
            placeholder='field name ...' className='outline-none
            border-b border-gray-200'/>
            <button type='button' onClick={handleAddField} className="flex
            items-center gap-2 text-sm text-gray-600 hover:text-gray-700
            cursor-pointer">
